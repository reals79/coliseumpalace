        ...
        $display->setTabs(function() {
            ...
            $main->setColumns([
                ...
                AdminColumn::custom('Действие', function(User $model) {
                    return (!empty($model->api_token)) ? '<p class="text-center"><form action="http://my.' . Request::getHttpHost() . '/login" role="form" method="POST" target="_blank">' . csrf_field() . '<input type="hidden" name="api_token" value="' . $model->api_token . '"><button type="submit">Вход</button></form></p>' : '';
                })->setWidth('100px'),
            ]);
